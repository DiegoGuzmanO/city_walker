...
gdf_shopping['geometry'] = gdf_shopping['geometry'].apply(lambda x: x.coords[0])

gdf_shopping =gdf_shopping.groupby('labels')['geometry'].apply(lambda x: Polygon(x.tolist())).reset_index()

gdf_shopping = gpd.GeoDataFrame(gdf_shopping, geometry = 'geometry')
...
shop_centroids=gpd.GeoDataFrame(gdf_shopping.centroid,columns=['geometry'])
...
for clusters in range(slider_clusters):
    ...
    gdf_shopping.loc[shop_centroids_index[clusters]].plot(color = 'orange',ax=ax,alpha=0.6)
